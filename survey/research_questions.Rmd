...
#  filter(Q_DEMOGRAPHICS_2 <= 2) %>%
  mutate(Q_GENDER = as_factor(Q_DEMOGRAPHICS_2)) %>%
  select('Q_PUPPET_INTEREST', 'ResponseId','v', "Q_AGE", "Q_DEMOGRAPHICS_2","Q_ROBOT_NARS","Q_TELEROBOT_WOULD_U_3") %>% 
  drop_na() 

  as_tibble(df %>% anova_test(Q_PUPPET_INTEREST ~ v * Q_ROBOT_NARS,detailed = TRUE,effect.size = "pes"))
...
df <- Telerobot_ages %>% select(starts_with("Q_GROUP_INTEREST"), 'Q_SHOW_INTEREST', 'Q_AGE', 'Q_DEMOGRAPHICS_2', 'ResponseId','v') %>% drop_na()
