...
ggdensity(Telerobot_ages %>% 
  select(Q_ROBOT_NARS, v) %>% 
    mutate(Q_ROBOT_NARS_INV = Q_ROBOT_NARS * -1) ,x = "Q_ROBOT_NARS", fill = "lightblue",facet.by = "v",xlab = "NARS value" )
...
ggdensity(Telerobot_ages %>% 
  ...
  ,x = "Q_GROUP_SLIDER_1", fill = "lightblue",facet.by = "v", xlab = "Feeling thermometer value toward the outgroup")
...
base_df <-  Telerobot_ages %>% select(matches("Q_TELEROBOT_DISP"), 'ResponseId','v','Q_GROUP_SLIDER_1') %>%    
  drop_na()
...
summary_fig <- ggsummarystats(
  melt(base_df %>% filter(Q_DEMOGRAPHICS_2 == 2) %>% plyr::rename(COLUMN_DICTIONARY), id=c('ResponseId','v'),variable.name = "Medium", value.name = "Interest"),
...
ggscatter(df, x = "Q_GROUP_INTEREST_4", y = "Q_GROUP_INTEREST_1",
          add = "reg.line",                                 # Add regression line
          conf.int = TRUE,                                  # Add confidence interval
          facet.by = "v",
          add.params = list(color = "blue",
...
```
### Did Israelis significantly prefer not puppet than puppet?
```{r}
df <- Telerobot_sorted %>% 
filter(v == 'IL') %>%
select(starts_with("Q_GROUP_INTEREST"),'v') %>%
