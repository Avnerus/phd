...
base_df <- Telerobot_ages %>% 
select(Q_ROBOT_NARS,v, Q_ROBOTS_SLIDER_4, Q_TECH_CONFIDENCE) %>% 
drop_na()
...
base_df <- Telerobot_ages %>% 
select(Q_GROUP_SLIDER_1,v,Q_AGE, Q_DEMOGRAPHICS_2) %>% Q_GROUP_
drop_na()
...
anova_test(base_df, Q_GROUP_SLIDER_1 ~ v, detailed = TRUE, effect.size = "pes")
...
anova_test(Q_TELEROBOT_WOULD_U_1 ~ v, data=base_df,effect.size = "pes")
anova_test(Q_TELEROBOT_WOULD_U_3 ~ v, data=base_df,effect.size = "pes")
...
map(~ anova_test(as.formula(paste0(.x," ~ v * Q_AGE * Gender")), data=df,effect.size = "pes"),c(test=.x))





anova_test(Q_TELEROBOT_WOULD_U_1 ~ v, data=df %>% filter(Gender == 'Female') ,effect.size = "pes")
...
gender_df <- melt(df %>% select(v,starts_with("Q_TELEROBOT_WOULD_U"),Gender) %>% plyr::rename(COLUMN_DICTIONARY),
 id=c('v','Gender'), variable.name = "Scenario") %>%
  ...
  summarise(across(c("value"), list(mean = ~mean(.x), n = ~ n(),sd)))
...
```{r}
  ...
  chisq.test()
   ...
   df <- Telerobot_ages %>% select("Q_TELEROBOT_APP_G", 'v') %>%    
  drop_na() %>%
  mutate(Human = ifelse(as_factor(Q_TELEROBOT_APP_G) == "A human.", 1, 0), Nonhuman = ifelse(as_factor((Q_TELEROBOT_APP_G)) == "A human.", 0, 1))  %>%
  group_by(v) %>%
  summarise(across(c('Human', 'Nonhuman'), list(sum = sum))) %>%
...
### Demographic redictors for specific features?
```{r}
...
### What if I take for each participant the value in relation to mean value given (in OUTGROUP)?
```{r, fig.width=10,fig.asp=0.5}
base_df <- Telerobot_ages %>% select(matches("^Q_TELEROBOT_FEAT_G_\\d$"), 'ResponseId','v') %>%    
  drop_na() 
...
base_df_m <- mean_df %>% select(matches("^Q_TELEROBOT_FEAT_G_\\d_M$"), 'ResponseId','v')
...
```{r, fig.width=10,fig.asp=0.5}
base_df <- Telerobot_ages %>% select(matches("^Q_TELEROBOT_FEAT_\\d$"),matches("^Q_TELEROBOT_FEAT_G_\\d$"), 'ResponseId','v') %>%    
  drop_na() 
...
feat_cols_g <- Telerobot_ages %>% select(matches("^Q_TELEROBOT_FEAT_G_\\d$")) %>% colnames
...
```{r, fig.width=10,fig.asp=0.5}
base_df <- Telerobot_ages %>% select("Q_TELEROBOT_DISP", 'ResponseId','v', 'Q_AGE', 'Q_DEMOGRAPHICS_2') %>%    
  drop_na() 
...
base_df <- Telerobot_ages %>% select("Q_TELEROBOT_DISP_G", 'ResponseId','v','Q_GROUP_SLIDER_1','Q_AGE','Q_DEMOGRAPHICS_2') %>% drop_na()
...
base_df <-  Telerobot_ages %>% select(matches("Q_TELEROBOT_DISP"), 'ResponseId','v','Q_GROUP_SLIDER_1', 'Q_AGE') %>%    
  drop_na()
...
base_df <-Telerobot_ages %>% select("Q_TELEROBOT_DISP_G", 'ResponseId','v','Q_TELEROBOT_APP_G') %>%    
  drop_na() %>%
...
df <- Telerobot_ages %>% select(starts_with("Q_TELEROBOT_WOULD_U_"), 'Q_PUPPET_INTEREST', 'ResponseId','v') %>% drop_na()
...
df <- Telerobot_ages %>% select(starts_with("Q_GROUP_INTEREST"), 'Q_SHOW_INTEREST', 'Q_AGE', 'Q_DEMOGRAPHICS_2', 'ResponseId','v','Q_DEMOGRAPHICS_2_4','Q_ROBOT_NARS') %>% drop_na()
