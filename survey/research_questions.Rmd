...
group_by(v) %>%
summarise(across(starts_w### Q: What was the distribution of tech confidence?
...
drop_na() %>%
anova_test(Q_TECH_CONFIDENCE ~ v + Q_AGE + Q_DEMOGRAPHICS_2, detailed=TRUE )
```ith("Q_DEMOGRAPHICS_2_"), list(freq=~sum(.x) / n() * 100,yes=~sum(.x), no = ~n()-sum(.x))))
...
```{r, fig.width=10,fig.asp=0.5}
  ...
  base_df <-Telerobot_ages %>% select('Q_PUPPET_INTEREST', 'ResponseId','v') %>% drop_na()
